...
void Eco::simulate_1_step(int iter){
	...
	static std::vector<std::vector<omp_lock_t>> lock(n,std::vector<omp_lock_t>(m));
	for(int i=0;i<n;++i)for(int j=0;j<m;++j) omp_init_lock(&lock[i][j]);
	
	// move rabbits
	std::vector<std::vector<std::vector<int>>> rab(n,std::vector<std::vector<int>>(m));
	...
	for(int i=0;i<n;++i){
	#pragma omp paralelize for
	for(int j=0;j<m;++j)if(!rab[i][j].empty()){
		...
		omp_set_lock(&lock[ni][nj]);
		if(grid[ni][nj]==NONE)grid[ni][nj]=RABBIT,proc[ni][nj]=-1;
		if(proc[ni][nj] < tproc ) proc[ni][nj]=tproc;
		omp_unset_lock(&lock[ni][nj]);
	}}
	
	std::vector<std::vector<std::vector<int>>> foxes(n,std::vector<std::vector<int>>(m));; // move foxes
	#pragma omp paralelize for
	for(int i=0;i<n;++i){
	#pragma omp paralelize for	
	...
	for(int i=0;i<n;++i){
	#pragma omp paralelize for
	for(int j=0;j<m;++j)if(!foxes[i][j].empty()){
